...
A type is a collection of zero or more fields, which are used to store data, and a collection of zero or more methods, which are used to perform operations. A type system is a specification of how types can be defined and used. A type system may also allow types to have relations, such as inheritance, between each other. In this case, a definition of a type must specify (in addition to fields and methods) how the type is related to all other types. This chapter provides an introduction to the Common Type System (CTS), which is the type system implemented by the Common Language Runtime (CLR).

An example of a type is **System.Int32**, which represents a 32-bit singed integer in the two&#39;s complement format. Types are crucial for writing correct, understandable code. If some code interpreted these 32 bits as a floating-point number, this would constitute a bug (because it violates the CTS) that may be difficult to detect. When you explicitly state that a value is of type **System.Int32**, compilers and runtimes can make sure that it&#39;s always used correctly, as a **System.Int32** value. This value is called an _instance_ of that type, which includes a state that can be accessed and manipulated by operations defined by the type<sup>1</sup>. In general, an instance consists of a state and the operations that can be performed on it.

Any instance has at least one type, where exactly one of its types fully describes it. For example, an instance of **System.Object** has one type that fully describes it, namely **System.Object**. An instance of **System.String** is also an instance of the parent type **System.Object**. However, in this case, **System.String** is called the _exact type_ or the _run-time type_ of the instance because it fully describes the fields of the instance and all the operations that can be performed on it. On the other hand, **System.Object** is a partial description of the instance.
...
For practical use cases of **TypedReference** and the relevant Visual C# operators, you can check out how they have been used to implement the **System.ArgIterator.GetNextArg** method group, the **GenericPtrToStructure\<T\>** and **GenericStructureToPtr\<T\>** methods defined in **System.Runtime.InteropServices.SafeBuffer** type, the **GetValue** and **SetValue** methods defined in the **System.Array** type, the **System.String.Concat** variadic method, the **WriteLine** and **Write** variadic methods defined in the **System.Console** type, the **Exchange\<T\>** and **CompareExchange\<T\>** methods defined in the **System.Threading.Interlocked** type, and the **System.Runtime.Serialization.ObjectManager.** **DoValueTypeFixup** method. I&#39;ll discuss some of these methods in the next section.
...
(1) Not all types in the CTS can be instantiated. For example, types, generic type parameters, and interfaces, all of which will be discussed in detail in later chapters, cannot be instantiated.
