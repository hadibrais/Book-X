...
All fields, local variables, and parameters that we have defined so far are _statically typed_. That is, their types are determined at compile-time. We say that types like **System.Object** , **System.Int32** , and **System.String** are static types. If we defined a local variable of type **System.Int32** , the C# compiler exactly knows the operators and methods that can be applied to this type. The compiler also knows those types whose instances can be assigned to that variable.  If, at compile-time, you attempted to call a method on an **Int32** instance that is not defined by **Int32** or one of its base types, the compiler will emit an error indicating that the method dos not exist. Similarly, if we attempted at compile-time to use an operator or access a field of a variable that is not provided by the type of that variable, the compiler will emit an error. And finally, if we attempted to assign an instance of **String** to an instance of **Int32** then the compiler will emit an error indicating that **String** cannot be assigned to **Int32**. The C# compiler emits these errors because it performs a process called _type checking_. This process inspects all constructs of your program including definitions, statements, and expressions and emits an error when it finds something that is considered by the compiler _semantically invalid_ (such as assigning **String** to **Int32** ).

The question is: why the C# compiler performs type checking? Let me rephrase this question as follows: what happens if the C# compiler did not perform type checking? Or in a third form: what happens at _run-time_ if the C# compiler did not perform type checking? Well, if the compiler allowed an expression that assigns a **String** to an **Int32** then when this code is executed, the CLR will perform type checking thereby detecting this inconsistency. Since this the CLR is a _type-safe runtime_, it will immediately stop the execution of this code and throw an exception. The reason for this is that the CLR is aware that it does not make any sense to assign a **String** to an **Int32**. If this was allowed, then programs will probably crash. Type safety is a mechanism that allows us to effectively shorten the application development life cycle by detecting bugs _as exceptions_ much earlier in the cycle. This is super awesome!

Now when the C# compiler detects that we are attempting to assign a **String** to an **Int32** , it knows for sure that the type-safe runtime will throw an exception. So instead of telling the programmer that he or she is attempting to do something that is not safe when the code executes, the compiler can also perform type checking to inform the programmer that this is not safe at compile-time. _Compile-time type-safety allows us to detect bugs that might be detected later at testing or might not be detected and escape even to the end user._ When the C# compiler emits an error about this assignment, we quickly know about this issue in our program and fix it without any difficulties.  All rules of type-safety that are respected by the runtime are optionally respected by the compiler. However, the compiler knows very little about the code we are writing. The compiler is able to detect that it is not safe to assign a **String** to an **Int32** because it knows everything about **String** , **Int32** , and the assignment operator. In many situations, the compiler cannot determine whether an operation is type-safe or not. Consider for the example the following code that involves an assignment:
...
**GetObject** is method that may be defined by you or by someone else. This method could contain any code that possibly involves various statements such as loops, if-else, and other method calls. At compile-time, the compiler does not know what type of object **GetObject** will return. All it knows that **obj** is a reference of type **Object** that refers to the returned object. The returned object could be an instance of any **Object** -derived type. It could be an **Int32** or a **String** or something that we never heard of. The compiler also knows that **integer** is of type **Int32**. Now when the compiler performs type checking on this code, it will see that we are attempting to assign an **Object** to an **Int32**. If the compiler can determine that **obj** references a boxed **Int32** all the time, then it can emit code that unboxes and copies this value to **interger**. However, since the compiler does not have enough information to make this guarantee, it determines that this is not safe and so it will emit the following error: **"Cannot implicitly convert type 'object' to 'int'. An explicit conversion exists (are you missing a cast?)"**. The compiler is telling us that I cannot guarantee that this conversion will succeed at run-time, and we have to use casting as follows:

```
Int32 integer = (Int32)obj;
```

When we use casting, we are stating to the compiler that we are responsible for the results of this conversion. Therefore, the compiler will respect our will and allows this code to be compiled. Now the type-safety of the runtime comes into play. The CLR does not even care whether the CIL code that it is executing is emitted by a type-safe language like C# or not. All that it cares about is that this code is type-safe. While the code is executing, the CLR has all information that it requires to ensure all aspects of type-safety. When **GetObject** is executed, it will return an instance of some type and a reference to this instance is stored in **obj**.  This means, when the casting is about to be executed, the CLR knows the type of this instance and can always determine whether this casting is safe or not and respond appropriately. For example, if **obj** refers to an instance of **String** then it is not safe to convert it to **Int32**. In this case, the CLR throws an **InvalidCastException** exception. But if **obj** refer to a boxed **Int32** then it is safe to convert it to **Int32**. In this case, the CLR would be more than happy to perform this conversion.

The CLR is the only component of .NET that must ensure type-safety by performing type checking. Other tools and compilers may also perform type checking _with various degrees_. For example, the C# compiler performs nearly full type checking. However, the C# compiler allows us to write code that is not guaranteed to be safe at compile-time by explicitly stating to the compiler that we are responsible of the results. The benefit of a type-safe compiler is that it effectively helps to write perfectly correct programs. Now pay attention to this. _The runtime being type-safe does not affect the syntax and the amount of code that we need to write to develop a program. In contrast, the compiler being type-safe does affect the syntax and the amount of code that we need to write._ That's the disadvantage of using a type-safe compiler. You have already seen an example of this where we assigned **obj** to **integer**. Since C# is type-safe, we needed to write more code by specifying casting to perform this assignment. If C# is not type-safe, then we don't need to explicitly write the casting syntax, resulting in simpler code.

From this, we can see that programming languages that are not type-safe could have some advantages that we cannot get from type-safe languages. The question is: Does it pay to develop _type-unsafe_ programming languages? Well, one advantage is the ability to write less and simpler code. But there are other significant advantages. Consider the following code:
...
At compile-time, we don't know the type of the object referenced by **obj**. We know that **obj** is a reference of type **Object**. We also know that **Object** does not provide an instance method called **Run**. In a type-safe language like C#, the compiler will emit an error indicating that the method does not exist. However, in a type-unsafe language, this code would compile with no errors. That's because the compiler does not care about type-safety. If you want to call a method that may not exist, then you can do it. However, at run-time, the CLR, as I have said before, does not care at all whether we compiled our code using type-safe compiler or not. It will enforce type-safety anyway. So assume that **GetObject** has returned an instance of **DateTime** , the CLR would then see that this type and all of its base types does not define an accessible instance method called **Run**. Therefore, the CLR will throw an exception.

Is it useful to write such code? Well, yes it is. We can for example easily develop an application using the top-down approach. So we can call methods that may have not yet been declared or implemented. We can also modify type definitions at run-time. For example, we could have a type that does not define any member but then at runtime, we can add more fields and methods when we need them. This technique could be used to have smaller and simpler types that we can expand or shrink as needed. Another useful scenario is when developing a domain-specific programming language (DSL) that is used by end users of an application. Ends users (who are usually not programmers) find it much more convenient to use a DSL without worrying about types of variables and conversions. In all of these scenarios, it is really better to have type-unsafe languages.

Note that whether we are using a type-safe language or a type-unsafe language, the runtime must always be type-safe. Otherwise, it will impossible to develop robust and secure systems and applications. This means that we have type-checking performed sooner or later. The difference between a type-safe language and a type-unsafe language is when type-checking is performed.

We use the term _compile-time type-checking_ when type-safety is ensured by the compiler. And we use the _term run-time type checking_ when type-safety is ensured by the runtime. For a programming language compiler/interpreter that performs type-checking, we say that this programming language is _static_. For a programming language compiler/interpreter that does not perform type-checking, we say that this programming language is _dynamic_. The term "static" means doing things at compile-time. The term "dynamic" means doing things at run-time.

A programming language being a static or dynamic is a matter of degree. This degree is determined by the degree of type-checking statically performed. More type-checking means that the language is more like static. Less type-checking means that the language is more like dynamic. Before .NET 4.0, the only way to write dynamic code is by using reflection. For example, the code attempts to call a method called **Run** that has two parameters one of type **String** and the other of type **Int32**. We would like to perform this call whether it is guaranteed that exists or not:
...
When compiling this code, the C# compiler will perform type-checking as usual. Since everything is okay as far as the compiler knows, the compiler will emit the CIL code with no complaints. That is, we have deferred the type-checking on the method **Run** until run-time. However, this code is not very convenient to an end user who does not know anything about programming. Unless you taught the end user how to use reflection in C#, the end user then will not be very happy using your app!

Dynamic programming languages allow us to write code very quickly without worrying about types of variables, fields, and parameters. We can simply declare a variable and assign to it a value like 10 or a value like "Some string". And then use these variables to perform some operations and call some methods to achieve some the required functionality. When this code is executed, type checking is performed by the runtime to make sure that everything is sound. If it finds that you are calling a method that is not provided by the type of an instance then the runtime will throw an exception. If it finds that you are passing an argument of type **String** to a parameter of type **Int32** , then it will throw an exception because there is no conversion from **String** to **Int32**. Examples of dynamic languages are Lisp, Smalltalk, PHP, Ruby, and Python.

The Dynamic Language Runtime (DLR) is a component of .NET that allows us to easily develop dynamic programming languages for the .NET framework. However, the DLR is capable of much more than that. We can also use the DLR to develop metaprograms which are programs that generate and manipulate programs. The DLR also allows us to develop scripting capabilities into applications to allow end users to perform complex tasks and automate them. Even more, DLR has a hosting API that allows code of one language to be executed within code of another language. For example, we can use IronPython code from IronRuby which in turn can be used from C#.  Note that the DLR not only enables us to implement dynamic langauges, but also enables us to let traditionally static languages like C# to provide dynamic features like the **dynamic** keyword.

The C# programming language is mostly static. However, it enables dynamic typing via the **dynamic** keyword. This will considerably simplify the syntax when interoperating with COM objects. This also enables us to host code of dynamic languages such as IronPython, meaning that using types and objects defined in IronRuby. The **dynamic** keyword is rarely used for other situations. The relation between the DLR and C# is packed into the **Microsoft.CSharp.dll** assembly and will be explained throughout this chapter. The important point is that this assembly references three more assemblies: **System.dll** , **System.Core.dll** , and **System.Dynamic.dll**. So when using the **dynamic** keyword, not only the performance will suffer because of late binding, but also more assemblies will be loaded consuming more memory.
...
Now let's examine the code in **Main**. Each time we call a method, the compiler has to determine which method that should be called. Similarly, each time we use an operator, the compiler has to determine which code that performs that required operation. This is not an easy task because methods and operators could be overloaded and so the compiler has to correctly figure out one of the overloads or emits an error when there is ambiguity. When we call **Add** and pass an instance of **Int32** , the compiler checks all overloads of **Add** and sees which one takes an **Int32**. Since we have such overload, the compiler will emit CIL code that calls this overload. Next, when we call **Add** and pass an instance of **String** , the compiler checks all overloads of **Add** and sees which one takes a **String**. Since we have such overload, the compiler will emit CIL code that calls this overload. Both of these cases are an example of _early binding_. Early method binding means that the resolution of a method call or an operator is done at compile-time. That is, the compiler is the one that determines and emits code to call a specific method.
...
Let's see how dynamic objects could affect the way we write conditional statements. In C#, all conditional statements must have one return type. That is, the returned values from all branches must be implicitly convertible to a single type. However, with the use of **dynamic** , we can have more flexibility. Consider for example the following code:
...
This example demonstrates how using **dynamic** can greatly simplify code. The problem is that we have an integer and we want to construct an array of type **Int32[]** when this integer is non-negative and do nothing otherwise. In each case, we need to call the appropriate overload of **Method** where one overload takes an **Int32[]** and the other takes an **Int32**. Another complexity arises when these overloads have difference return types. One returns a **String** and the other returns an **Int32**. We want to apply the + operator on the return value and pass it to **Console.WriteLine**. Without **dynamic** , we have to write code for each case because we cannot use conditional statements and the ternary operator when different branches return values of incompatible types. The reason for this is that the following code that depends on which branch is taken would be type-unsafe. With **dynamic** , we can skip compiler type checking and use the ternary operator even though each branch returns a value of a different type.

### 11.2.2 Dynamic Type Conversion

A dynamically typed variable has a type that is determined at run-time. All method calls and operators on dynamic variables are dynamically dispatched. The same idea is also applied to dynamic type conversion. It is easy to see that any statically typed variable (except for unmanaged pointers) can be implicitly converted to a dynamically type variable because the compiler treats **dynamic** as if it was **Object**. We can also implicitly convert a dynamically typed variable to a statically typed variable. Here is an example that illustrates this:
...
When the compiler sees that we are assigning a variable of type **Object** to a variable of type **Int32** , it determines that the success of this conversion is not always guaranteed. So an error is emitted. To fix this, we need to explicitly convert **Object** to **Int32** indicating that we take responsibility of the results of the conversion. The compiler now loosens type checking and allows this cast to be compiled, relying on the CLR to detect invalid conversions. However, when converting a dynamically typed local variable to a statically typed local variable, we are stating to the compiler that this is a dynamic conversion and it should skip type checking on it. So the compiler will allow this implicit conversion. As usual, the runtime will throw an exception when type-safety violation is attempted.

### 11.2.3 ExpandoObject

We have seen that the dynamic type enables us to create dynamically-typed variables so we can perform various operations on these variables such as accessing members, calling methods, and performing arithmetic. However, we can do even more. The DLR provides a type called **ExpandoObject** that enables us to add and remove members dynamically. This is interesting because there is no way in C# to add or remove members from an object. The internal details of **ExpandoObject** will be explained later in this chapter. In fact, we will develop a similar type called **MiniExpandoObject** in section 11.5.3.1 "Example: MiniExpandoObject."
...
Now users of our expression **ReadFileExpression2** can easily modify the file name by calling **Update** passing the new file name represented by a constant expression.
...
Figure 11.5: Shows what happens when we site.Target is invoked ignoring cache lookup. Step 2 of the internal static method is magnified to see the block expression constructed by the DLR.
...
### 11.6.4 Things We Can't Do With Dynamic Typing

I mentioned that the **dynamic** type is a compile-time, meaning that it does not exist at run-time. I have also said that the **dynamic** type is a type and can be used almost anywhere any other type can be used. In the following few sections, I will discuss where we cannot use the **dynamic** type.

**You Cannot Derive From dynamic**

The first place where we cannot use the **dynamic** type is inheritance. So we cannot have a type that derives from **dynamic**. If we compiled the following definition:

```
internal class Foo : dynamic { }
```

Then the compiler will emit the following error: " **'Foo': cannot derive from the dynamic type**". Even though the compiler treats **dynamic** as **Object** , we can derive from **Object** but not from **dynamic**. This is one of the few situations where the compiler differs between these types. The reason for this is that it does not make any sense. What does it mean to derive from **dynamic**? If it means the same thing as deriving from **Object** then the compiler should emit an error to eliminate the confusion. But is seems weird to have a type deriving from **dynamic** anyway.

**You Cannot Implement Dynamic Interfaces**

The second place where we cannot use the **dynamic** type is when implementing generic interfaces. So we cannot specify one of the type arguments to be **dynamic**. If we compiled the following definition:

```
internal class Foo : IComparable<dynamic> { }
```

Then the compiler will emit the following error: "**'Foo': cannot implement a dynamic interface 'System.IComparable<dynamic>'**". There are two reasons for this. First we cannot apply the **DynamicAttribute** to type arguments (but we can apply them to type parameters). So we will not be able to differentiate between implementing **IComparable<dynamic>** and **IComparable<Object>**. The second reason is that it is not useful. That's because you can still implement the interface while casting generic parameters to **dynamic** whenever you want to perform a dynamic operation. We can also have references of type **IComparable<dynamic>**.

By the way, although we cannot implement dynamic interfaces, we can derive from generic types and specify one or more of the generic parameters to be **dynamic**. The **DynamicAttribute** will be applied here to the derived type. This is possible here because a type can only have one base type.

**You Cannot Use dynamic in Type Parameter Constraints**

The third place where we cannot use the **dynamic** type is in generic parameter constraints. So if we compiled the code below, the compiler will emit the following error: " **Constraint cannot be the dynamic type**".

```
public class Foo<T> where T : dynamic { }
```

It does not make much sense to constraint a type parameter to be **dynamic**. If we want to accept only dynamic objects then we can constrain the type parameter to implement the **IDynamicMetaObjectProvider** interface.

**You Cannot Write Extension Methods for dynamic**

The last place where we cannot use the **dynamic** type is specifying it as the first parameter of an extension method. So if we compiled the code below, the compiler will emit the following error: " **The first parameter of an extension method cannot be of type 'dynamic'**".

```
public static void ExtendDynamic(this dynamic arg) { }
```

Just as with the previous section, it does not make much sense to extend the **dynamic** type. If we want to provide an extension method for dynamic objects then we can extend the **IDynamicMetaObjectProvider** interface.
